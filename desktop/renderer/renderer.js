...
async function openSettings() {
  ...
  try {
    ...
    panel.append(element("h2", "", "Модель, провайдер и оформление")); panel.append(element("p", "", "Ключи сохраняются только в системном защищённом хранилище. Каталог выбранного провайдера загружается live и доступен для поиска."));
    ...
    const modelSearch = element("input", ""); modelSearch.placeholder = "Поиск по каталогу моделей выбранного провайдера";
    ...
    const loadModels = async () => {
      ...
      catalogNote.textContent = result.modelCatalogSource === "live" ? `Live Hugging Face каталог: ${result.modelCatalogTotal} моделей, показано ${models.length}.` : result.modelCatalogSource === "provider" ? `Live каталог выбранного провайдера: ${result.modelCatalogTotal} моделей, показано ${models.length}.` : result.modelCatalogSource === "provider-unavailable" ? "Каталог провайдера недоступен. Проверьте Base URL и API key; ID модели можно ввести вручную." : `Встроенный список: ${models.length}. Проверьте токен или сеть для полного каталога.`;
    };
    ...
    provider.onchange = () => { const next = providerDefaults(provider.value); if (provider.value !== "custom") endpoint.value = next.endpoint || ""; modelSearch.value = ""; modelResults.replaceChildren(); catalogNote.textContent = "Нажмите Применить, затем каталог выбранного провайдера будет загружен live."; };

...
export const TOKEN_MODES = ["economy", "balanced", "quality"];

function configDirectory() {
  if (process.platform === "win32") return path.join(process.env.APPDATA || path.join(homedir(), "AppData", "Roaming"), "HuggingCode");
...
function providerId(value) {
  const id = String(value || "huggingface").trim().toLowerCase();
  return /^[a-z0-9-]{2,40}$/.test(id) ? id : "huggingface";
}
...
function normalizeConfig(stored = {}) {
  return {
    ...
    tokenMode: TOKEN_MODES.includes(stored.tokenMode) ? stored.tokenMode : "balanced",
    provider: providerId(stored.provider),
    providerEndpoint: endpoint(stored.providerEndpoint),
    theme: typeof stored.theme === "string" && stored.theme.trim() ? stored.theme.trim() : "ember",
...
export function getConfigLocation() { return configPath(); }
export function getConfigDirectory() { return configDirectory(); }

...
export class CodingAgent {
  ...
  getContextStats() {
    ...
    return {
      ...
      characters,
      estimatedTokens: estimateTokens(" ".repeat(characters)),
      byRole,
    };
  }
  ...
  async requestCompletion(messages, tools = this.workspace.toolDefinitions) {
    const request = {
      model: this.model,
      messages,
      tools,
    ...
    try {
      const completion = await this.client.chatCompletion(request);
  ...
  async ask(userMessage) {
    ...
    for (let round = 0; round < MAX_TOOL_ROUNDS; round += 1) {
      ...
      const completion = await this.requestCompletion(this.messages);
  ...
  async compact(focus = "") {
    ...
    const summaryRequest = [
      ...
      {
        role: "user",
        content: `Summarize the following conversation for continued coding work. Preserve user goals, decisions, exact files changed, command outcomes, unresolved issues, and constraints. Omit secrets. ${focus ? `Focus on: ${focus}` : ""}\n\nConversation:\n${original.map((message) => `${message.role}: ${getTextContent(message.content)}`).join("\n\n")}`,
      },
    ];
    const completion = await this.requestCompletion(summaryRequest, []);

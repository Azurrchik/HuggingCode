...
export function StatusBar({ status, theme }) {
  ...
  const modeTone = status.mode === "plan" ? theme.info : status.mode === "manual" ? theme.warning : status.mode === "safe-auto" ? theme.success : theme.accent;
  const state = status.busy ? "working" : status.queueCount ? `queued: ${status.queueCount}` : "ready";
  ...
  h(Box, { justifyContent: "space-between", flexWrap: "wrap" },
    h(Box, null,
      ...
      h(Text, { dimColor: true }, ` · effort ${status.effort}`),
      h(Text, { dimColor: true }, " · "),
      h(Text, { color: modeTone, bold: true }, status.mode),
      h(Text, { dimColor: true }, " · "),
      h(Text, { color: status.busy ? theme.warning : theme.success }, state),
    ),
    h(Box, null,
      h(Text, { dimColor: true }, "ctx "),
      h(Text, { color: contextTone, bold: true }, percent === null ? "?" : `${percent}%`),
      h(Text, { dimColor: true }, ` · tokens ${formatCount(status.usage?.totalTokens)}`),
    ),
  ),
  h(Box, null,
    h(Text, { dimColor: true }, status.workspace || "workspace не определён"),
    status.goal ? h(Text, { color: theme.info }, ` · goal: ${status.goal}`) : null,
  ));
}
